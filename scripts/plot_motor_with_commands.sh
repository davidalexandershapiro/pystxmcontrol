#!/bin/bash
# Script to plot motor positions with overlaid command information using OperationLogger
# Usage: ./plot_motor_with_commands.sh <motor_name> [date] [--save path]
#
# Arguments:
...
#
# Examples:
#   ./plot_motor_with_commands.sh SampleX
#   ./plot_motor_with_commands.sh Energy 2025-10-27
#   ./plot_motor_with_commands.sh SampleX 2025-10-27 --save /tmp/combined_plot.png
...
# Create Python script to plot motor with commands
...
# Plot motor with commands
try:
    fig = logger.plot_motor_with_commands(
    ...
    if fig is None:
        ...
        if save_path:
            ...
            print(f"Displaying combined plot for motor '{motor_name}' with commands...")
        sys.exit(0)

except Exception as e:
    print(f"Error plotting motor with commands: {e}", file=sys.stderr)
    import traceback
    traceback.print_exc()
    sys.exit(1)
EOF

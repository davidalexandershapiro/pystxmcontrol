#!/bin/bash
# Script to plot motor positions using OperationLogger
# Usage: ./plot_motor_positions.sh <motor_name> [date] [--save path]
#
# Arguments:
...
#   ./plot_motor_positions.sh SampleX 2025-10-27 --save /tmp/plot.png
...
# Create Python script to plot motor positions
...
try:
    fig = logger.plot_motor_positions(
    ...
    print(f"Error plotting motor positions: {e}", file=sys.stderr)
    import traceback
    traceback.print_exc()
    sys.exit(1)
EOF

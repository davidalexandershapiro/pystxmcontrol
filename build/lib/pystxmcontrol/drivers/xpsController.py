...
class xpsController(hardwareController):
    ...
    def __init__(self, address = '192.168.168.253', port = 5001, simulation = False):
        ...
        self.stopped = False
        self.moving = False
        xpsController.__nbSockets = 0
        for socketId in range(self.MAX_NB_SOCKETS):
            xpsController.__usedSockets[socketId] = 0

    def initialize(self, simulation = False):
        self.simulation = simulation
        if not(self.simulation):
            print("Connecting to XPS socket...", self.address, self.port)
            self.controlSocket = self.TCP_ConnectToServer(self.address, self.port, 1)
            self.monitorSocket = self.TCP_ConnectToServer(self.address, self.port, 1)

    # Send command and get return
    def __sendAndReceive(self, socketId, command):
        try:
            xpsController.__sockets[socketId].send(command.encode())
            ret = xpsController.__sockets[socketId].recv(1024).decode()
            while (ret.find(',EndOfAPI') == -1):
                ret += xpsController.__sockets[socketId].recv(1024)
    ...
    def TCP_ConnectToServer(self, IP, port, timeOut):
        socketId = 0
        if (xpsController.__nbSockets < self.MAX_NB_SOCKETS):
            while (xpsController.__usedSockets[socketId] == 1 and socketId < self.MAX_NB_SOCKETS):
                socketId += 1
        ...
        xpsController.__usedSockets[socketId] = 1
        xpsController.__nbSockets += 1
        try:
            xpsController.__sockets[socketId] = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            xpsController.__sockets[socketId].connect((IP, port))
            xpsController.__sockets[socketId].settimeout(timeOut)
            xpsController.__sockets[socketId].setblocking(1)
        except socket.error:
            return -1
    ...
    def TCP_SetTimeout(self, socketId, timeOut):
        if (xpsController.__usedSockets[socketId] == 1):
            xpsController.__sockets[socketId].settimeout(timeOut)
    ...
    def TCP_CloseSocket(self, socketId):
        if (socketId >= 0 and socketId < self.MAX_NB_SOCKETS):
            try:
                xpsController.__sockets[socketId].close()
                xpsController.__usedSockets[socketId] = 0
                xpsController.__nbSockets -= 1
            except socket.error:
                pass
    ...
    def ControllerMotionKernelTimeLoadGet(self, socketId):
        if (xpsController.__usedSockets[socketId] == 0):
            return
    ...
    def GroupMoveAbsolute(self, socketId, GroupName, TargetPosition):
        err,currentPos = self.GroupPositionCurrentGet(socketId, GroupName, 1)
        moveDelta = abs(TargetPosition[0] - currentPos)
        if (xpsController.__usedSockets[socketId] == 0):
            return
        ...
        while self.moving:
            err,currentPos = self.GroupPositionCurrentGet(socketId, GroupName, 1)
            positionErr = TargetPosition[0] - currentPos
            if abs(positionErr) > 0.2: #0.05 * moveDelta:
                if not(self.stopped):
                    if (time.time() - t0) > 30.0:
                        print("XPS move timeout. Aborting...")
                        self.moving = False
                        return self.GroupMoveAbort(socketId, GroupName)
                    else:
                        time.sleep(0.1)
                else:
                    self.moving = False

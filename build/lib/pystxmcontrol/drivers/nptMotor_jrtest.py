...
class nptMotor(motor):
    ...
    def __init__(self, controller = None):
        ...
        self.trajectory_stop_x = 2
        self.trajectory_start_y = -0.5
        self.trajectory_stop_y = 1
    ...
    def update_trajectory(self):
        print('RUN UPDATE NOW')
        x_range = abs(self.trajectory_start_x - self.trajectory_stop_x)
        y_range = abs(self.trajectory_start_y - self.trajectory_stop_y)
        ...
        print('pad_x, pad_y from driver level:', self.xpad, self.ypad)
        print('velocity x, y', self._xVelocity, self._yVelocity)
    ...
    def moveLine(self, direction = 1):
        ...
        if self.lineMode == 0:
            ...
            if not(self.simulation):
                ##this is only for horizontal lines and needs to be made general
                print("BEFORE UPDATE tr_start_x, tr_stop_x", self.trajectory_start_x, self.trajectory_stop_x)
                print("BEFORE UPDATE tr_start_y, tr_stop_y", self.trajectory_start_y, self.trajectory_stop_y)
                self.update_trajectory()
                print("tr_start_x, tr_stop_x", self.trajectory_start_x, self.trajectory_stop_x)
                print("tr_start_y, tr_stop_y", self.trajectory_start_y, self.trajectory_stop_y)
                x0,y0 = self.trajectory_start_x, self.trajectory_start_y
                x1,y1 = self.trajectory_stop_x, self.trajectory_stop_y

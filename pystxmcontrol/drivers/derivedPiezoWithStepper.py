...
class derivedPiezoWithStepper(motor):
    ...
    def moveCoarse(self, pos):
        ...
        #the coarse motor only has about 6 micron precision so we clean up the slop with the piezo
        #this is using the coarse encoder though and not the interferometer
        deltaPos = pos - self.axes["axis2"].getPos()
        if self.config["reset_after_move"]:
            self.axes["axis1"].setPosToZero()
            self.axes["axis1"].servoState(True)
